  ...
  return (
    <SafeAreaView style={styles.loginContainer}>
      ...
      <View style={styles.loginBox}>
        ...
        />
        {/* Login button */}
        <TouchableOpacity style={styles.loginButton} onPress={handleFirebaseLogin}>
...
function MapScreen({ navigation, route }) {
  ...
  const saveTelemetrySample = async (sample) => {
    ...
    await addDoc(collection(db, "telemetry", user.uid, "samples"), {
      ...
      weight_lb: sample.weight_lb ?? null,
  ...
  useEffect(() => {
    ...
    if (wasConnected && !espConnected && !disconnectNotificationSentRef.current) {
  ...
  /* comment in later when distance is done
  // Earth radius for simple meter->degree conversion (WGS84 approximate)
  const R_EARTH = 6378137; // meters

  // Convert ESP-provided X/Y (meters) to latitude/longitude using a reference point.
  // Assumes x = East (m), y = North (m). If `headingDeg` is provided and x/y are
  // in a rover-local frame, the function will rotate them into the global ENU frame.
  const espXYToLatLon = (x, y, refLat, refLon, headingDeg = 0) => {
    let xg = x;
    let yg = y;
    if (headingDeg) {
      const t = (headingDeg * Math.PI) / 180;
      xg = x * Math.cos(t) - y * Math.sin(t);
      yg = x * Math.sin(t) + y * Math.cos(t);
    }
    const dLat = (yg / R_EARTH) * (180 / Math.PI);
    const dLon = (xg / (R_EARTH * Math.cos((refLat * Math.PI) / 180))) * (180 / Math.PI);
    return { latitude: refLat + dLat, longitude: refLon + dLon };
  };
  // NOTE: espXYToLatLon is currently disabled (commented out). Re-enable when
  // ESP telemetry x/y (meters) is finalized and you want automatic conversion.
  */
  ...
  useEffect(() => {
    ...
    const pollTelemetry = async () => {
      ...
      try {
        ...
        setEspError("");
        // Update rover fields from ESP32 data.
        // NOTE: x/y (meter) conversion is currently disabled — keep simple lat/lon
        // parsing for now. The previous x/y->lat/lon code is preserved below
        // (commented) for future re-enabling.
        setRoverData((prev) => ({
          ...prev,
          latitude: data?.lat ?? data?.latitude ?? prev.latitude,
          longitude: data?.lon ?? data?.longitude ?? prev.longitude,
          // raw_x/raw_y and x/y->latlon conversion intentionally disabled
          battery: data?.battery_pct ?? prev.battery,
          weight_lbs: data?.weight_lb ?? prev.weight_lbs,
          mode: mapEspMode(data?.mode) ?? prev.mode,
        }));

        /*
        // Previous implementation (disabled): convert meter-based x/y -> lat/lon
        setRoverData((prev) => {
          let latitude = prev.latitude;
          let longitude = prev.longitude;
          if (typeof data?.x === "number" && typeof data?.y === "number") {
            const refLat = userLocation?.latitude ?? prev.latitude;
            const refLon = userLocation?.longitude ?? prev.longitude;
            const headingDeg = data?.heading ?? 0;
            const ll = espXYToLatLon(data.x, data.y, refLat, refLon, headingDeg);
          ...
          return {
            ...
            latitude,
            longitude,
            raw_x: data?.x ?? prev.raw_x,
            raw_y: data?.y ?? prev.raw_y,
            battery: data?.battery_pct ?? prev.battery,
            weight_lbs: data?.weight_lb ?? prev.weight_lbs,
            mode: mapEspMode(data?.mode) ?? prev.mode,
          };
  ...
  // Autonomous simulation removed — real rover movement provided by ESP

  // Go Home simulation removed — Go Home feature disabled
  ...
  // Send one manual control command to ESP32
  ...
  // Manual-mode local simulation removed — manual inputs only send commands to the ESP
  ...
  return (
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.infoBoxBottom}>
        ...
        <Text style={styles.infoText}>Weight: {roverData.weight_lbs.toFixed(2)} lbs</Text>
        <Text style={styles.infoText}>Battery: {roverData.battery}%</Text>
